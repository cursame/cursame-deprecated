class Api::ApiController < ApplicationController
  ...
  def user
    ...
    if @user.nil? or token.nil?
      logger.info("Token not found.")
      render :status => 404, :json => {:message => "Invalid token", :token => token}
    else
      render :status => 200, :json => {:user => @user.as_json, :count => 1, :token => token }
    end
  end
  ...
  def notifications
    ...
    respond_to do |format|
      format.json { render :json => {:notifications => @notifications.as_json, :count => @notifications.count()} }
    end
  end
  ...
  def comments
    ...
    respond_to do |format|
      format.json { render :json => {:comments => ActiveSupport::JSON.decode(@comments.to_json(:include => [:user, :comments])), :count => @comments.count()} }
    end
  end
