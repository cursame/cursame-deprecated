...
class SupervisorController < ApplicationController
  ...
  def students
    @students = current_network.students.where(:view_status => 'live').order("upper(first_name), upper(last_name) asc").page(params[:page]).per(10000)
  ...
  def supervisors
    @supervisors = current_network.supervisors.order("upper(first_name), upper(last_name) asc").page(params[:page]).per(300)
    respond_to do |format|
      format.html
